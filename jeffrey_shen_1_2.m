...
for u  = 1:length(tau)
    ...
    %Free Evolution
    B_n = wgn(1,length(rotateTime),-80);

    for t = 1:length(rotateTime) - 1
        U = expm(-1i * (H0 + B_n(t)*gamma*S_z) *dt2);
        psi0 = U*psi0;
    end
     
    %Rotate 90 Degrees
    B_n = wgn(1,length(time2),-120);
...
ylim([-0.5 0.5])
xlabel ('Tau (s)');
ylabel('Spin Expectation Value Along Z');
